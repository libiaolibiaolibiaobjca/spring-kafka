...
## Purpose
TBD - created by archiving change release-3-3-16-nes-patch-1. Update Purpose after archive.
## Requirements
### Requirement: Exclusive and safe component preparation

The release process MUST assign one active owner to `spring-kafka-3.3` and MUST inspect branch, origin, tracked and untracked changes, and active OpenSpec changes before modifying `spring-kafka-3.3`.
...
Every generated RELEASE POM or BOM for `3.3.16-nes.patch.1` MUST use approved RELEASE versions for internal `cn.bjca.footstone` dependencies, parents, imported BOMs, and plugins.
...
The component SHALL have one dedicated release commit containing `3.3.16-nes.patch.1`, internal RELEASE dependency updates, `README.md`, `doc/QUICK_START.md`, `doc/USER_MANUAL.md`, `doc/GAV_MAPPING.md`, approved exclusions, and no unrelated source changes.
...
- **WHEN** no POM, binary, checksum, metadata, or partial module asset exists for `3.3.16-nes.patch.1`
...
After Nexus verification, the coordinator MUST create annotated tag `v3.3.16-nes.patch.1` on the exact release commit used to build and deploy `3.3.16-nes.patch.1`.
