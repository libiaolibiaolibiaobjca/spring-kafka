...
`spring-kafka-3.3` is a Git repository at `spring-kafka-3.3`. This change releases `3.3.16-nes.patch.1` from one dedicated source commit and records enough evidence for a fresh session to reconcile the component against Git, OpenSpec, and Nexus.
...
- Publish `3.3.16-nes.patch.1` with no internal `cn.bjca.footstone` SNAPSHOT metadata.
...
- Verify immutable remote assets before creating annotated tag `v3.3.16-nes.patch.1`.
...
Only the manifest owner for `spring-kafka-3.3` may write to this repository while the component is being prepared.

### 2. Publish only a complete internal RELEASE dependency graph

Set the project version to `3.3.16-nes.patch.1` and update every internal dependency, parent, imported BOM, and plugin version to its approved RELEASE. Generate the complete local publication set and scan all generated POM/BOM metadata, not only the representative GAVs.
...
After the one authorized deploy, download representative POM and binary assets from Nexus RELEASE, verify version and metadata, calculate checksums, and run the RELEASE-only consumer smoke test. Do not create or push `v3.3.16-nes.patch.1` until the component reaches `nexus-verified`.
...
Create annotated tag `v3.3.16-nes.patch.1` on the exact release commit used for deployment. Verify the remote branch and tag after push.
