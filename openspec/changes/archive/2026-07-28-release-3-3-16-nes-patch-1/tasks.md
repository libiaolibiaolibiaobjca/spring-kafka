...
- [x] 1.1 Confirm this change is apply-ready and assigned exclusively to `spring-kafka-3.3` in `spring-kafka-3.3`.
- [x] 1.2 Acquire the component owner lease and record owner/session/timestamps in the release manifest.
- [x] 1.3 Record branch, `origin` fetch/push URLs, HEAD SHA, active OpenSpec changes, toolchain, current version, and target `3.3.16-nes.patch.1` without exposing credentials.
...
- [x] 2.1 Set the component version to `3.3.16-nes.patch.1`.
...
- [x] 2.3 Discover the complete local publication set and reconcile it with the representative GAVs: `cn.bjca.footstone.bpring.kafka:bjca-footstone-bpring-kafka:3.3.16-nes.patch.1`.
...
- [x] 4.3 Create the dedicated release commit for `spring-kafka-3.3` `3.3.16-nes.patch.1`.
...
- [x] 7.1 Confirm `v3.3.16-nes.patch.1` does not already exist locally or remotely and the target release commit SHA is unchanged.
- [x] 7.2 Create annotated tag `v3.3.16-nes.patch.1` on the exact release commit only after `nexus-verified`.
