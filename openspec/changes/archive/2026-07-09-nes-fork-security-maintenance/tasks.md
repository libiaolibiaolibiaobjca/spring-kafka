...
- [x] 1.7 向用户汇报调研结论与状态分布，确认后再进入升级阶段（已汇报，确认采用「按升级后✅记录」）
...
- [x] 2.5 更新 VULNERABILITY_REPORT 中因升级而转为「✅已修复」的 kafka CVE 状态（5 个 kafka CVE 已按 3.9.2 升级后 ✅ 记录）
...
- [x] 6.2 向用户提交变更影响分析总结与最终状态，确认后归档 openspec change
